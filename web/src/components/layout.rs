...
pub fn Layout(children: ChildrenFn) -> impl IntoView {
    ...
    view! {
        <ErrorBoundary fallback=|errors| {
            ...
            <div
                class="flex justify-center items-center h-svh lg:h-screen overflow-hidden overflow-y-hidden bg-black lg:bg-[linear-gradient(rgba(0,0,0,0.75),rgba(0,0,0,0.75)),var(--bg-image)] lg:bg-cover lg:bg-center lg:bg-no-repeat"
                style=move || format!("--bg-image: url('{}')", random_background.get())
            >
                <div
                    class="h-svh absolute top-0 lg:top-[30px] bottom-0 w-full lg:h-[calc(100%-60px)] lg:w-[600px] bg-neutral-950 lg:rounded-3xl transition-all duration-150 flex flex-col"
